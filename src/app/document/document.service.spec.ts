...
describe("DocumentService", () => {
	...
	describe("#get()", () => {
		it("should resolve with a array of valid branches when apiService returns success", (done: DoneFn) => {
			const branchDoc1 = {
				documentName: "branch",
				data: { name: "MikkyD", id: "abc" } as Branch,
			};
			const branchDoc2 = {
				documentName: "branch",
				data: { name: "Bking", id: "x" } as Branch,
			};
			const returnObj = { data: [branchDoc1, branchDoc2] };
	...
	describe("#update()", () => {
		it("should resolve with the document if apiService resolves with success", (done: DoneFn) => {
			const doc = {
				documentName: "aDoc",
				data: { title: "aDocumentTitle", id: "abc" },
			};
			...
			service
				.update("collection", "abc", { hello: "there" })
				.then((resDoc: any) => {
		...
		it("should reject with BlApiError if the document returned from ApiService is invalid", (done: DoneFn) => {
			const doc = {
				documentName: "aDoc",
				data: { missing: "this doc is missing an id field" },
			};
		...
		it("should reject with BlApiLoginRequiredError if apiService throws it", (done: DoneFn) => {
			...
			service
				.update("collection", "abc", { hello: "my man" })
				.catch((blApiError: BlApiError) => {
					expect(
						blApiError instanceof BlApiLoginRequiredError
					).toBeTruthy();
					done();
				});
		});
	...
	describe("#add()", () => {
		...
		it("should reject with BlApiPermissionDeniedError if apiService throws it", (done: DoneFn) => {
			...
			service
				.add("collection", { hello: "my man" })
				.catch((blApiError: BlApiError) => {
					expect(
						blApiError instanceof BlApiPermissionDeniedError
					).toBeTruthy();
					done();
				});
		});
	...
	describe("#remove()", () => {
		...
		it("should reject with BlApiNotFoundError if apiService throws it", (done: DoneFn) => {
			...
			service
				...
				.catch((blApiError: BlApiError) => {
					expect(
						blApiError instanceof BlApiNotFoundError
					).toBeTruthy();
					done();
				});
		});
